...
public class JwtTokenProvider {
    ...
    // 유효시간
    private int jwtExpirationInM = 24 * 60 * 60 * 1000;     // 1일을 ms 단위로 계산
    
    public String createToken(Authentication authentication) {
        UserDetails userPrincipal = (UserDetails) authentication.getPrincipal();
        Date expiryDate = new Date(System.currentTimeMillis() + jwtExpirationInM);  // 이 내용으로 유효시간 설정
        
        SecretKey secretKey = Keys.hmacShaKeyFor(SECRET_KEY.getBytes());    // 이 내용으로 전자서명에 사용할 해시값 생성됨
        return Jwts.builder()
